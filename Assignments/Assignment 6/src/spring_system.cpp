...
void SpringSystem::generate_constraint_row(Eigen::MatrixXd &C, int row_index, int node_id, const std::vector<int> &free_dof_indices)
{
    ...
    float theta = node.constraint_angle * static_cast<float>(M_PI) / 180.0f;
    float normal_angle = theta + static_cast<float>(M_PI) / 2.0f;

    float a_x = std::cos(normal_angle);
    float a_y = std::sin(normal_angle);
...
int SpringSystem::solve_system()
{
    ...
    for (int i = 0; i < num_nodes; ++i)
    {
        ...
        if (nodes[i].constraint_type == Slider)
        {
            float theta = nodes[i].constraint_angle * static_cast<float>(M_PI) / 180.0f;
    ...
    for (auto &spring : springs)
    {
        ...
        spring.stress = static_cast<float>(axial_force / spring.A / 1e6); // MPa
